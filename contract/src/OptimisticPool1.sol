...
contract ProvingPool {
    ...
    uint256 constant RING_SIZE = 69; // claim batches will start getting overwritten after circling for RINGSIZE time
    uint256 constant BATCH_FINALIZATION_TIME = 12 hours;
    ...
    uint256 constant BLESSING_TIME = 1 hours; // can be relatively short
    uint256 constant CHALLENGE_TIME = 7 hours;
    ...
    mapping(uint256 => ClaimBatch) private claimRingQueue; // this will keep RINGSIZE amount of batches of length at most 256

    mapping(uint256 => Blessing) private blesserQueue; //
    mapping(uint256 => address) private kindQueue; // list of checker contracts, appendable by anyone
}
